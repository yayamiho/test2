    ...
    */
    ...
    public function deleteSubmission($data, $user) {
        ...
        try {
            ...
            if(!isset($data['submission'])) $submission = Submission::where('status', 'Draft')->where('id', $data['id'])->first();
            elseif($data['submission']->status == 'Pending') $submission = $data['submission'];
            else $submission = null;
            if(!$submission) throw new \Exception("Invalid submission.");
            if($user->id != $submission->user_id) throw new \Exception("This is not your submission.");
